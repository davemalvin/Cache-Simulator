...
struct architectural_state {
    int state; // State of the current instruction's execution (e.g. INSTR_FETCH or DECODE)
    uint64_t clock_cycle; // current clock cycle since the start of execution
    struct ctrl_signals control; // current values of control signals
    struct instr_meta IR_meta;
    struct pipe_regs curr_pipe_regs; // used within a cycle to read value of a given register
    struct pipe_regs next_pipe_regs; // stores value that needs to be written into at the end of the cycle
    int bits_for_cache_tag;
    struct memory_stats_t mem_stats; // memory stats
    int registers[REGISTER_NUM]; // Array of registers; Models a register file
